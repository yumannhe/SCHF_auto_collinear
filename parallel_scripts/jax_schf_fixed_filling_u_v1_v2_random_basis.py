...
SCHF parallel run:
'''
res = schf_fixed_filling_prallel_u_v(schf_single_job, Htb, a_lists, phase_pos, phase_neg, dict_ref, input_d_tot, input_bond_tot, filling, u_arr, v_arr, temperature)

np.savez_compressed(
    os.path.join(project_root, "u_v1_filling_half_t_4_em4_random_basis_mesh_60.npz"),
    u=np.asarray(u_arr),
    v=np.asarray(v_arr),
    d=np.asarray(jax.device_get(res["d"])),
    bond=np.asarray(jax.device_get(res["bond"])),
    e_diff=np.asarray(jax.device_get(res["e_diff"])),
    c_diff=np.asarray(jax.device_get(res["c_diff"])),
    gse=np.asarray(jax.device_get(res["gse"])),
    e_fermi=np.asarray(jax.device_get(res["e_fermi"])),
    any_bi_fail=np.asarray(jax.device_get(res["any_bi_fail"])),
    iters=np.asarray(jax.device_get(res["iters"])),
)
